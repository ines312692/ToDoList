        ...
        stage('Deploy to Kubernetes') {
			steps {
                ...
                withKubeConfig([
                    ...
                    serverUrl: 'https://192.168.49.2:65333'
                ]) {
					sh '${WORKSPACE}/bin/kubectl apply -f k8s/backend-deployment.yaml'
                    sh '${WORKSPACE}/bin/kubectl apply -f k8s/frontend-deployment.yaml'
                    sh '${WORKSPACE}/bin/kubectl wait --for=condition=ready pod -l app=todo-backend --timeout=300s'
                    sh '${WORKSPACE}/bin/kubectl wait --for=condition=ready pod -l app=todo-frontend --timeout=300s'
                    sh '${WORKSPACE}/bin/kubectl get pods'
                    sh '${WORKSPACE}/bin/kubectl get services'
                }
            }
        }
    }
